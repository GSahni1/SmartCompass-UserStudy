...
public class NavigationController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            // stop the NavMesh agent to walk to destination
    ...
    // 🔥 HANDLES DESTINATION ARRIVAL (CALLED FROM POI.Arrived)
    public void ArrivedAtDestination()
    {
        // Notify MultiSet listeners
        DestinationArrived.Invoke();

        // ✅ END FIREBASE SESSION HERE (CORRECT METHOD SIGNATURE)
        if (NavigationSessionManager.Instance != null)
        {
            // Distance will be wired next; steps handled internally
            NavigationSessionManager.Instance.EndSession(0f);
        }
